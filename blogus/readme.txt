=== Blogus ===
Contributors: Themeansar
Author: Themeansar
Requires PHP: 7.4
Requires at least: 6.7
Tested up to: 7.0
Stable tag: 3.0.3
Version: 3.0.3
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html
Tags: one-column, two-columns ,right-sidebar, flexible-header, custom-background, custom-header, custom-menu, editor-style, featured-images, footer-widgets,  theme-options, threaded-comments, rtl-language-support, translation-ready, full-width-template, custom-logo, blog, news

== Description ==

Blogus is a fast, clean, modern-looking Best Responsive News Magazine WordPress theme. The theme is fully widgetized, so users can manage the content by using easy to use widgets. Blogus is suitable for dynamic news, newspapers, magazine, publishers, blogs, editors, online and gaming magazines, newsportals,personal blogs, newspaper, publishing or review siteand any creative website. Blogus is SEO friendly, WPML,Gutenberg, translation and RTL ready. Live preview : https://demos.themeansar.com/blogus-demos/ and documentation at https://docs.themeansar.com/docs/blogus/ 

== Installation ==

1. In your admin panel, go to Appearance -> Themes and click the 'Add New' button.
2. Type in Blogus in the search form and press the 'Enter' key on your keyboard.
3. Click on the 'Activate' button to use your new theme right away.

== Copyright ==

Blogus WordPress Theme, Copyright (c) 2025, Themeansar
Blogus is distributed under the terms of the GNU GPLs

== Credits ==

* CSS bootstrap.css
Code and documentation copyright 2011-2016 Twitter, Inc. Code released under the MIT license. Docs released under Creative Commons.

* JS bootstrap.js
Copyright 2011-2018 The Bootstrap Authors (https://github.com/twbs/bootstrap/graphs/contributors)
https://github.com/twbs/bootstrap/blob/master/LICENSE

* Bootstrap navwalker
* Copyright (C) 2007 Free Software Foundation, Inc. <http://fsf.org/>
https://github.com/twittem/wp-bootstrap-navwalker


* Google Fonts  
* Josefin Sans
    License: [Google Fonts Licensing](https://fonts.google.com/knowledge/glossary/licensing)  
    Source: [Josefin Sans on Google Fonts](https://fonts.google.com/specimen/Josefin+Sans)

* Rubik
    License: [Google Fonts Licensing](https://fonts.google.com/knowledge/glossary/licensing)  
    Source: [Rubik on Google Fonts](https://fonts.google.com/specimen/Rubik)

== Font Awesome license ==
======================================
https://github.com/FortAwesome/Font-Awesome FontAwesome 5.15.1 Copyright 2012 Dave Gandy, Travis Chase, Rob Madole, Brian Talbot, Jory Raphael, Mike Wilkerson, Trevor Chase, Jason Lundien, Jason Otero, Edward Emanuel, Geremia Taglialatela
Font License: SIL OFL 1.1 Code License: MIT License https://github.com/FortAwesome/Font-Awesome/blob/master/LICENSE.txt

* Customizer Pro *
Author: Justin Tadlock
URL:  https://github.com/justintadlock/trt-customizer-pro
License: GPLv2 or later

* jQuery Cookie * 
Author:  Klaus Hartl, Copyright © 2006-2014 
URL: https://github.com/carhartl/jquery-cookie
License: MIT

== Smartmenu ==
======================================
 * Owner : https://github.com/vadikom
 * Copyright (c) Vasil Dinkov, Vadikom Web Ltd. ( https://github.com/vadikom/smartmenus/blob/master/LICENSE-MIT)
 * Licensed under https://github.com/vadikom/smartmenus
 * https://github.com/vadikom/smartmenus/blob/master/src/addons/keyboard/jquery.smartmenus.keyboard.js

* Screenshot Banner Image - https://pxhere.com/en/photo/1446003
* https://negativespace.co/abstract-flowing-laser-dark/
License: CC0 Public Domain

== Post Image ==

License: CC0 Public Domain
License URI: https://stocksnap.io/license
License URI: https://pxhere.com/en/license

* https://pxhere.com/en/photo/1588583
* https://pxhere.com/en/photo/1363072
* https://pxhere.com/en/photo/1262676
* https://pxhere.com/es/photo/1574183
* https://pxhere.com/en/photo/994444
* https://pxhere.com/en/photo/1087277
* https://pxhere.com/en/photo/1574895
* https://pxhere.com/en/photo/32966
* https://pxhere.com/en/photo/525075
* https://pxhere.com/en/photo/857626
* https://pxhere.com/en/photo/72451
* https://stocksnap.io/photo/colorful-buildings-QJTRQQWWL0

== Changelog ==
= Version 0.1
* Initial release.

= Version 0.2
* Fixed theme review issue.

= Version 0.3
* Fixed theme review issue.

= Version 0.4
* Update Theme URI.
* Update Theme Description.

= Version 0.5
* Fixed styling issue.

= Version 0.6
* Update strings.

= Version 0.7
* Update strings.

= Version 0.8
* Fixed styling issue.
* Remove unused code.

= Version 0.9
* Added blog content layout.

= Version 1.0.0.1
* Fixed styling issue.

= Version 1.0.0.2
* Fixed styling issue.

= Version 1.0.0.3
* Added footer menu.
* Fixed styling issue.

= Version 1.0.0.4
* Fixed meta live editing issue.

= Version 1.0.0.5
* Fixed styling issue.

= Version 1.0.0.6
* Fixed styling issue.

= Version 1.0.0.7
* Fixed styling issue.

= Version 1.0.0.8
* Fixed styling issue.

= Version 1.0.0.9
* Fixed column & styling issue.

= Version 1.0.0.10
* Added Bootstrap RTL.

= Version 1.0.0.11
* Fixed styling issue.

= Version 1.0.0.12
* Fixed styling issue.

= Version 1.0.0.13
* Fixed styling issue.

= Version 1.0.0.14
* Fixed styling issue.

= Version 1.0.0.15
* Fixed styling issue.

= Version 1.0.0.16
* Fixed table styling issue.

= Version 1.0.0.17
* Fixed styling issue.

= Version 1.0.0.18
* Fixed woo block styling issue.
* Fixed woo page breadcrumb issue.

= Version 1.0.0.19
* Fixed .bs-model class issue.

= Version 1.0.0.20
* Fixed .bs-target class issue.

= Version 1.0.0.21
* Fixed styling issue.

= Version 1.0.0.22
* Fixed RTL styling issue.

= Version 1.0.0.23
* Fixed styling issue.
* Added WordPress 6.2 compatibility 

= Version 1.0.0.24
* Fixed styling issue.

= Version 1.0.0.25
* Fixed styling issue.
* Fixed escaping issue.

= Version 1.0.0.26
* Remove unused code.

= Version 1.0.0.27
* Fixed styling issue.

= Version 1.0.0.28
* Added Border color img select.

= Version 1.0.0.29
* Fixed styling issue.

= Version 1.0.0.30
* Fixed Full-Width page layout styling.

= Version 1.0.0.31
1. Remove Unnecessary Code.

= Version 1.0.0.32
1. Remove Unnecessary Code.
2. Added Print button,
3. Added # for tag.

= Version 1.0.0.33
1. Fixed Tag issue.
2. Fixed date issue.

= Version 1.0.0.34
1. Fixed Category double tag issue.

= Version 1.0.0.35
1. Fixed block style issue.
2. Fixed selective refresh issue.

= Version 1.0.0.36
1. Fixed fonts issue.

= Version 1.0.0.37
1. Remove Unnecessary Code.

= Version 1.0.0.38
1. Added Page layout setting.

= Version 1.0.0.39
1. Fixed underline link issue.

= Version 1.0.0.40
1. Fixed Heading and Styling issue.

= Version 1.0.0.41
1. Fixed sanitization and translate string.

= Version 1.0.0.42
1. Fixed lite and dark tuggle issue.

= Version 1.0.0.43
1. Fixed styling issue.

= Version 1.0.0.44
1. Fix sidebar width on archive
2. hide next pagination on grid layout

= Version 1.0.0.45
1. Fixed Bredcrumb issue

= Version 1.0.0.46
1. Fixed Category div issue.

= Version 1.0.0.47
1. Fixed string issue.
2. Fixed styling issue.

= Version 1.0.0.49
1. Remove unnecessary code.

= Version 1.0.0.50
1. Fixed styling issue.

= Version 1.0.0.51
1.Thumbnail support on page.
2. Added edit link on page.

= Version 1.0.0.52
1. Fixed color sanitization.

= Version 1.0.0.53
1. Fixed Custom Background color issue on dark mode.
2. Fixed post share by WhatsApp

= Version 1.0.0.54
1. Added Featured Image Caption.

= Version 1.0.0.55
1. Fixed D/N switch On site title hide.

= Version 1.0.0.56
1. Added breadcrumb on single page.

= Version 1.0.0.57
1. Added Featured post widget.

= Version 1.0.0.58
1. Added Customize Button on Demos Import Banner.
2. Fixed styling issue.

= Version 1.0.0.59
1. Fixed Two H1 Tag issue.
2. Added alt in Author Image.

= Version 1.0.0.60
1. Fixed js issue.

= Version 1.0.0.61
1. Fixed Single Page Styling.
2. Fixed Single Page RTL Styling.

= Version 1.0.0.62
1. Added Navigation in Search.

= Version 1.0.0.63
1. Fixed Gravatar image issue.

= Version 1.0.0.64
1. Fixed Page image & title issue.

= Version 1.0.0.65
1. Added Twitter X.

= Version 1.0.0.66
1. Added Page Title.

= Version 1.0.0.67
1. Fixed Author Widget.

= Version 1.0.0.68
1. Fixed Responsive Issue.

= Version 1.0.0.69
1. Fixed RTL Styling.

= Version 1.0.0.70
1. Fixed RTL Styling Issue.
2. Fixed Styling Issue.

= Version 1.0.0.71
1. Fixed Woocommerce Page Width issue.

= Version 1.0.0.72
1. Fixed Styling Issue. 

= Version 1.0.0.73
1. Update Screenshot

= Version 1.0.0.74
* Fixed Blog Post Content.

= Version 1.0.0.75
* Added Gutenberg Widget Style.

= Version 1.0.0.76
1. Fixed Styling Issue. 

= Version 1.0.0.77
1. Removed Unnecessary Code.

= Version 1.0.0.78
1. Fixed styling issue.

= Version 1.0.0.79
1. Fixed Issue on Dark Mode.
2. Fixed Featured Slider

= Version 1.0.0.80
1. Fixed Typography Control.
2. Fixed Post Meta Function.

= Version 1.0.0.81
1. Fixed Recent Posts Image Option.

= Version 1.0.0.82
1. Fixed Color in Dark Mode on Woocommerce.

= Version 1.0.0.83
1. Add Edit Icon on Content layout.
2. Fixed Content Code. 

= Version 1.0.0.84
1. Fixed Footer Column Refresh Issue.
2. Fixed RTL Styling Issue.

= Version 1.0.0.85
1. Add Selective refresh to site identity.

= Version 1.0.0.86
1. Added Selective Refresh to Home Page Layouts.

= Version 1.0.0.87
1. Fixed styling issue.

= Version 1.0.0.88
1. Fixed slider issue.

= Version 1.0.0.89
1. Added Selective Refresh to sidebar width and footer logo size.

= Version 1.0.0.90
1. Remove Unnecessary Code.
2. Fixed Custom Color CSS.

= Version 1.0.0.91
1. Make function for menu right nav buttons.

= Version 1.0.0.92
1. Fixed Archive Page H1 Issue.

= Version 1.0.0.93
1. Fixed Archive Page H1 Issue.
2. Fixed Tabs Keyword Issue on Mobile.

= Version 1.0.0.94
1. Fixed Dark and light Switcher Issue.

= Version 1.0.0.95
1. Fixed Archive Page and Pages Title Issue.

= Version 1.0.0.96
1. Fixed Translating Issue Issue.
2. Add functionality code in hook in search and footer template. 
3. remove unused code and files.

= Version 1.0.0.97
1. Fixed Styling Issue.

= Version 1.0.0.98
1. Fixed Text Domain Issue.
2. Fixed Comments Label.

= Version 1.0.0.99
1. Fixed Text Domain Issue.

= Version 2.0.0.0
1. Added selective refresh to header social icons.

= Version 2.0.0.1
Added Page Title For Woocommerce.

= Version 2.0.0.2
* Fixed Page Title Issue.

= Version 2.0.0.3
* Fixed Styling Issue.

= Version 2.0.0.4
* Fixed Styling Issue.

= Version 2.0.0.5
* Fixed Tab Keyword Issue.

= Version 2.0.0.6
* Fixed Comments Translating.

= Version 2.0.0.7
1. Added selective refresh to header right nav.

= Version 2.0.0.8
1. Added selective refresh to footer.

= Version 2.0.0.9
1. Added selective refresh to footer social icons.

= Version 2.0.1.1
1. Added selective refresh to featured slider title.

= Version 2.0.1.2
1. Fixed Translating Issue.

= Version 2.0.1.3
1. Added selective refresh to Footer Copyright.

= Version 2.0.1.4
1. Added selective refresh to scroll to top icon.

= Version 2.0.1.5
1. Manage Unwanted Space.
2. Remove Unused Code.

= Version 2.0.1.6
1. Added Customizer Preview Function.

= Version 2.0.1.7
1. Added Selective Refresh on Theme Color.

= Version 2.0.1.8
1. Added Posts Image Height Option.

= Version 2.0.1.9
1. Remove Unnecessary Code.
2. Remove Unnecessary Space.

= Version 2.0.2.0
1. Fixed Styling Issue.

= Version 2.2.1
1. Added Selective Refresh on Single Page.

= Version 2.2.2
1. Added Selective Refresh on Single Page.

= Version 2.2.3
1. Fixed Background Image Issue.

= Version 2.2.4
1. Fixed Translating Issue.

= Version 2.2.5
1. Fixed Comments label Issue.

= Version 2.2.6
1. Added Selective Refresh to Featured Links.

= Version 2.2.7
1. Added Function on Featured Links.

= Version 2.2.8
1. Added Selective Refresh to Featured Links.

= Version 2.2.9
1. Added Selective Refresh to Footer Copyright.

= Version 2.3.0
1. Fixed Full Width Page Issue.

= Version 2.3.1
1. Fixed Full Width Page Bredcrumb Issue.

= Version 2.3.2
1. Fixed Author Widget Image JS.

= Version 2.3.3
1. Fixed Customize Image URL.

= Version 2.3.4
1. Added Default In Author Widget.

= Version 2.3.5
1. Fixed Featured Post List Styling Issue.

= Version 2.3.6
1. Fixed Full Width Page Image Issue.

= Version 2.3.7
1. Fixed Full Width Page Edit Issue.
2. Fixed Features Links URL Issue.

= Version 2.3.8
1. Fixed Styling Issue.

= Version 2.3.9
1. Fixed RTL Styling Issue.

= Version 2.4.0
1. Added Selective Refresh to You Missed Section.

= Version 2.4.1
1. Added Heading Font Variable.

= Version 2.4.2
1. Fixed Featured Slider JS.

= Version 2.4.3
1. Fixed Footer Issues.

= Version 2.4.4
1. Added Body Font Variable.

= Version 2.4.5
1. Fixed Styling Issues.

= Version 2.4.6
1. Fixed Search Page Result Issue.

= Version 2.4.7
1. Added Search Result Count.

= Version 2.4.8
1. Fixed Styling Issue.

= Version 2.4.9
1. Fixed Search Page Styling Issue.

= Version 2.5.0
1. Added Image Resolution Class in Images.

= Version 2.5.1
1. Fixed Styling Issues of Sidebar Widgets.

= Version 2.5.2
1. Added Google Fonts Licence

= Version 2.5.3
1. Added Starter Sites URL.

= Version 2.5.4
1. Added Notice Links.

= Version 2.5.5
1. Fixed Typography Fonts.

= Version 2.5.6
1. Fixed Notice.

= Version 2.5.7
1. Update Notice.

= Version 2.5.8
1. Fixed Sidebar Comments Styling.

= Version 2.5.9
1. Fixed Footer List Styling Issues.

= Version 2.6.0
1. Fixed Footer List Styling Issues.

= Version 2.6.1
1. Fixed Comments.

= Version 2.6.2
1. Fixed Widgets Issues.

= Version 2.6.3
1. Fixed Single Page Spacing Issue.

= Version 2.6.4
1. Fixed Category Styling Issue.

= Version 2.6.5
1. Fixed Meta Styling Issues.

= Version 2.6.6
1. Fixed Author Widget Issues.

= Version 2.6.7
1. Fixed Footer menus Styling Issues.

= Version 2.6.8
1. Fixed Inputs RTL Styling Issues.

= Version 2.6.9
1. Fixed Category Count Styling.

= Version 2.7.0
1. Fixed Security Issues.

= Version 2.7.1
* Added Theme Support Function.

= Version 2.7.2
* Fixed Inputs Color Issue on Dark Mode.

= Version 2.7.3
* Added Woocommerce Category on Products.

= Version 2.7.4
* Fixed Product Meta Issue on Search Page

= Version 2.7.5
* Fixed WooCommerce Product Page Styling Issues.

= Version 2.7.6
* Added Navigation On Product Featured Image.

= Version 2.7.7
* Fixed WooCommerce Styling Issues.

= Version 2.7.8
* Fixed WooCommerce Product Price Color Issues.

= Version 2.7.9
* Fixed Shop Page Pagination Styling.

= Version 2.8.0
* Fixed Post Except Lines Issue.

= Version 2.8.1
* Added Theme Color Variable.

= Version 2.8.2
* Added Cursor Dot
* Added Preloader 
* Added Schema Markup

= Version 2.8.3
* Fixed Color Variable Issue.

= Version 2.8.4
* Fixed Dropdown Icons Issue on Responsive.

= Version 2.8.5
* Fixed Styling Issue.

= Version 2.8.6
* Fixed RTL Styling Issue.

= Version 2.8.7
* Fixed Post Sharing Issues.

= Version 2.8.8
* Fixed Google Fonts Load.

= Version 2.8.9
* Fixed Footer Overlay Image Issue.

= Version 2.9.0
* Fixed RTL Styling Issue.

= Version 2.9.1
* Fixed Styling Issue.

= Version 2.9.2
* Fixed Admin Notice.

= Version 2.9.3
* Fixed RTL Styling Issue.

= Version 2.9.4
* Fixed Post Description.

= Version 2.9.5
* Expanded localization with new translation files.
* Added multi-language support for German, Russian, Spanish and Dutch.

= Version 2.9.6
* Fixed Styling Issue of Product Single Page.

= Version 2.9.7
* Fixed Admin Notice.

= Version 2.9.8
* Update Screenshot.

= Version 2.9.8
* Inhanced WP Color Variables in Customizer.

= Version 3.0.0
* Added Customizer options to disable breadcrumbs on archive, single, page, and search views.
* Update Screenshot.

= Version 3.0.1
* Added Visibility Option For Header Right Nav.

= Version 3.0.2
* Improved single post title markup.
* Updated default slider title font size.

= Version 3.0.3
* Added Site Title and Tagline visibility options.